...
export function parseText(rawText: string, type: 'poetry' | 'prose' | 'drama'): ParsedText {
	...
	if (type === 'poetry') {
		return { type: 'poetry', poems: parsePoetry(lines) };
	} else if (type === 'prose') {
		return { type: 'prose', chapters: parseProse(lines) };
	} else if (type === 'drama') {
		return { type: 'drama', acts: parseDrama(lines) };
	}

	throw new Error(`Unsupported text type: ${type}`);
}

function parsePoetry(lines: string[]): Poem[] {
	let globalIndex = 0;
	let charOffset = 0;
	const poems: Poem[] = [];
...
function parseProse(lines: string[]): ProseChapter[] {
	let globalIndex = 0;
	let charOffset = 0;
...
function parseDrama(lines: string[]): DramaAct[] {
	let globalIndex = 0;
	let charOffset = 0;
	const acts: DramaAct[] = [];
