...
<div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
	<h3 class="text-sm font-semibold text-gray-900 mb-4">Add New Text</h3>
	{#if error}
		<div class="mb-4 bg-red-50 text-red-600 text-sm rounded-lg px-3 py-2">
			{error}
		</div>
	{/if}
	<form onsubmit={handleSubmit}>
				...
				{#if showCategorySuggestions && existingCategories.length > 0}
					...
					>
						{#each existingCategories.filter((c) => c
							...
							<button
								type="button"
								class="w-full text-left px-3 py-1.5 text-sm hover:bg-primary-50 transition-colors"
								onmousedown={(e) => {
									e.preventDefault();
									categoryValue = cat;
									showCategorySuggestions = false;
		...
		<button
			type="submit"
			disabled={submitting}
			class="px-4 py-2 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-lg shadow-sm transition-colors disabled:opacity-50"
		>
